...
/// <summary>Value converter for converting hexadecimal strings to a SolidColorBrush.</summary>
public class HexStringToSolidColorBrushConverter : IValueConverter
{
    ...
    {
        var text = (value ?? parameter)?.ToString();
        if (text is null || text.Length < MinimumRgbLength)
        {
            return new object();
        }
